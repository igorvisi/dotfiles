...
echo "#######################"
sudo usermod -aG autologin,nopasswdlogin,wheel,input,vboxsf,vboxusers,docker $USER
...
for packageOff in $(cat ~/.extra/mega.co.nz/packages/pacman.txt); do sudo pacman -S $packageOff --needed --noconfirm ; done

echo "##########################"
echo "Install packages from AUR"
echo "##########################"
for packageAUR in $(cat ~/.extra/mega.co.nz/packages/aur.txt); do yay -S $packageAUR --needed ; done
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
	...
	echo "###################################"
	for extension in $( cat ~/.extra/mega.co.nz/packages/vscode.txt ); do code --install-extension $extension ; done
fi
...
if [[ $REPLY =~ ^[Yy] ]]; then
	...
	echo "############################"
	for package in $( cat ~/.extra/mega.co.nz/packages/npm.txt ); do npm install -g $package ; done
fi

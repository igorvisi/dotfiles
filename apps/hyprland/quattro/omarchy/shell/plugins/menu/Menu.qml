...
Item {
  ...
  readonly property string configHome: Quickshell.env("OMARCHY_CONFIG_HOME") || Quickshell.env("HOME") + "/.config/omarchy"
  // JSONC menu definitions. The shell parses both at startup and merges
  // the user file on top of the defaults, so the keybind → IPC → visible
  // path doesn't have to shell out to bash + jq on every open.
  property string defaultMenuPath: omarchyPath + "/default/omarchy/omarchy-menu.jsonc"
  property string userMenuPath: configHome + "/extensions/omarchy-menu.jsonc"
  ...
  function finishRequest(selection) {
    ...
    resultProc.requestSubmitted = true
  ...
  Process {
    ...
    onExited: {
      if (!resultProc.requestSubmitted) return
      resultProc.requestSubmitted = false

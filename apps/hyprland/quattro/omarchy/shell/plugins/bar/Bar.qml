...
Item {
  ...
  property string stateHome: Quickshell.env("OMARCHY_STATE_HOME") || home + "/.local/state/omarchy"
  property string omarchyConfigDir: Quickshell.env("OMARCHY_CONFIG_HOME") || home + "/.config/omarchy"
  ...
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f ${OMARCHY_STATE_HOME:-$HOME/.local/state/omarchy}/toggles/bar-off ]] && echo yes || echo no"]

...
# - Run: `# curl -sL https://git.io/igorvisi-install | bash`
...
mkdir -p /mnt/{mnt/btrfs-root,boot/efi,home,var/{cache/pacman,log,tmp,lib/docker,.snapshots}
...
echo "ln_CD.UTF-8 UTF-8" >> /mnt/etc/locale.gen
ln -sf /usr/share/zoneinfo/Africa/Kinshasa /mnt/etc/localtime
...
for group in wheel wheel input vboxsf sambashare vboxusers docker ; do
